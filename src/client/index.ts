...
const en = {
  ...
  description: '.env file management, switch, validate',
  enabled: 'Enable plugin',
  backupEnabled: 'Enable backup',
  envFile: 'Config file path',
};
...
function Card(props: any) {
  ...
  return React.createElement('li', { className: `dsh-${NS}-card`, style: s },
    ...
    open ? React.createElement('div', { style: { padding: '8px 0', borderTop: '1px solid #333' } },
      ...
      React.createElement('label', { style: { ...label, marginTop: '6px' } },
        React.createElement('input', { type: 'checkbox', checked: scope?.get?.('backupEnabled') ?? true, onChange: (e: any) => scope?.set?.('backupEnabled', e.target.checked) }),
        t('backupEnabled'),
      ),
    ) : null,
  );
